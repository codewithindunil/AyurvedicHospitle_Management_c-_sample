...
{
    public partial class ShopForOthers : UserControl
    {
        int count = 0;
        double netTotal=0;
        String max;
        ...
        public ShopForOthers()
        {
            InitializeComponent();
        }

        private void ShopForOthers_Load(object sender, EventArgs e)
        {
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            string[] row = { order_id, patient_name, iteme_id, item_name, qty, each_price, total };
            count = new Shop().sell(order_id, item_name, iteme_id, qty, each_price, total, "", patient_name);
            if (count == 1)
            {
